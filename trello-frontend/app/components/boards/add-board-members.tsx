...
import { getNoBoardMembers, addMemberToBoard } from '../../services/boardService';
import { Board, User } from '../../types';
...
const AddBoardMembers = ({ board }: { board: Board }) => {
  ...
  // Fetch non-members for the board
  const getMembers = async () => {
    try {
      const response = await getNoBoardMembers(Number(board.id)); // Fetch non-members using Spring Boot backend
      setMembers(response);
    } catch (error) {
  ...
  // Add an existing member to the board
  const addMembers = async (user: User) => {
    try {
      await addMemberToBoard(Number(board.id), user.email); // Use addMemberToBoard from Spring Boot service
      toast.success(`Member ${user.name} added successfully!`);
  ...
  // Send an invitation via email (simple logging for demo)
  const sendInvitation = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);

    try {
      // Use the Spring Boot backend to send an email invitation (mock logic here)
      console.log(`Send invitation to email: ${email} for board: ${board.title}`);
      toast.success('Invitation sent successfully!');
      setEmail('');
    } catch (error) {
      toast.error('Failed to send invitation. Please try again.');
