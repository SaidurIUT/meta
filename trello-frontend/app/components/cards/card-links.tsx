...
import { addCardLink, removeCardLink } from '../../services/cardService';
import { Card } from '../../types';
...
const CardLinks: FC<CardLinksProps> = ({ cardData, onCardUpdate }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [links, setLinks] = useState<string[]>([]);
  const formRef = useRef<HTMLFormElement>(null);
  const [isEditable, setIsEditable] = useState(false);
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent): Promise<void> => {
      ...
      try {
        const updatedLinks = [...links, newLink];
        const updatedCard = { ...cardData, links: updatedLinks };

        setLinks(updatedLinks);
        await onCardUpdate(updatedCard);

        const res = await addCardLink(cardData.id, newLink);
  ...
  const handleRemove = useCallback(
    async (index: number): Promise<void> => {
      try {
        ...
        const res = await removeCardLink(cardData.id, links[index]);
              ...
              links.map((link: string, index: number) => (
                ...
                >
                  <a
                    href={link}
                    target="_blank"
                    className="flex-1 truncate text-xs hover:text-blue-700"
                  >
                    {link}
                  </a>
                  <button
