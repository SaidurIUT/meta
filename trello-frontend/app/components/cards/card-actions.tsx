...
// import { copyCard, deleteCard } from '../../services/cardService';
...
//   const boardId = params.boardId; // Convert boardId to number
...
//     if (!cardData?.id || !boardId) return;
  
//     if (confirm('Are you sure you want to delete this card?')) {
//       try {
//         const res = await deleteCard({ cardId: cardData.id, boardId:boardId });
//         if (res.success) {
...
const CardActions: FC<CardActionsProps> = ({
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      const copiedCard = await copyCard({ id: cardData.id, boardId });
      // Assuming copyCard returns the copied CardDTO
      await onCardUpdate(copiedCard);
      toast.success('Card successfully copied');
    } catch (error: any) {
      console.error('Error copying card:', error);
      toast.error('Failed to copy card');
    }
  }, [cardData?.id, boardId, onCardUpdate]);
  ...
  const handleDelete = useCallback(async () => {
    ...
    if (confirm('Are you sure you want to delete this card?')) {
      try {
        await deleteCard({ cardId: cardData.id, boardId });
        toast.success('Card deleted');
