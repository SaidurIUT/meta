...
import { addCardTodo, changeTodoCompleted, removeCardTodo } from '../../services/cardService';
import { Card, Todo } from '../../types';
...
const CardTodo = ({ cardData, setCardData }: CardProps) => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        const newTodo = { content: todoContent, completed: false };
        const updatedCard = await addCardTodo(cardData.id, newTodo);
  ...
  const handleCompleted = useCallback(
    async (todoId: string) => {
      ...
      try {
        ...
        const updatedCard = await changeTodoCompleted(cardData.id, todoId, !todo.completed);
        setCardData(updatedCard);
        toast.success('Todo status updated');
      } catch (error) {
        console.error('Error updating todo status:', error);
  ...
  const handleRemove = useCallback(
    async (todoId: string) => {
      ...
      try {
        if (confirm('Are you sure you want to delete this todo?')) {
          const updatedCard = await removeCardTodo(cardData.id, todoId);
          setCardData(updatedCard);
          toast.success('Todo deleted');
        }
      } catch (error) {
    ...
    <div className="mb-[1rem]">
      <div>
        <SubTitle
          ...
          length={todos.length}
        />
        <div className={`no-scrollbar max-h-[40vh] overflow-auto ${isOpen ? 'block' : 'hidden'}`}>
          <div className="my-2">
            {todos.length > 0 &&
              todos.map((todo: Todo, index: number) => (
                <div
