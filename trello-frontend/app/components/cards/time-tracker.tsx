...
import { updateCardTrackedTimes } from '../../services/cardService'; // Updated to match backend API
import { Button } from '../ui/button';
import { Card } from '../../types';
...
export default function TimeTracker({ cardData }: TimeTrackerProps) {
  ...
  const handleStartStop = async () => {
    if (isTracking) {
      const newTrackedTime = formatTime(currentTime);
      const updatedTimes = [...trackedTimes, newTrackedTime];
      setTrackedTimes(updatedTimes);
      setCurrentTime(0);

      try {
        await updateCardTrackedTimes(Number(cardData.id), updatedTimes); // Updated API call
       
        localStorage.removeItem(STORAGE_KEYS.isTracking(cardData.id));
        localStorage.removeItem(STORAGE_KEYS.startTime(cardData.id));
        router.refresh();
      } catch (error) {
        console.error('Error updating tracked times:', error);
