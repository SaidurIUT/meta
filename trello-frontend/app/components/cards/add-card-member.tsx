...
import { getNoCardMembers, updateCard } from '../../services/cardService';
import { Card, User } from '../../types';
...
interface CardProps {
  card: Card;
  boardId: string; // Updated to number
  children?: React.ReactNode;
  onCardUpdate?: (card: Card) => Promise<void>;
}

const AddCardMember = ({ card, boardId, children, onCardUpdate }: CardProps) => {
  const [members, setMembers] = useState<User[]>([]);

  useEffect(() => {
    const getMembers = async () => {
      try {
        const response = await getNoCardMembers(boardId, card.id); // Spring Boot service call
        if (response) {
          setMembers(response);
        }
      } catch (error) {
  ...
  const handleSubmit = useCallback(
    async (user: User) => {
      try {
        ...
        // API update using Spring Boot backend
