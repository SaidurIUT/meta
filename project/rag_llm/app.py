...
# Save team context
@app.route('/save/team/<context_id>', methods=['POST'])
def save_team(context_id):
    return save_generic_context(context_id, "team")

# Save office context
@app.route('/save/office/<context_id>', methods=['POST'])
def save_office(context_id):
    return save_generic_context(context_id, "office")

# Save task context
@app.route('/save/task/<context_id>', methods=['POST'])
def save_task(context_id):
    return save_generic_context(context_id, "task")

# Save document context
@app.route('/save/doc/<context_id>', methods=['POST'])
def save_doc(context_id):
    return save_generic_context(context_id, "doc")

# Generic function to save context
def save_generic_context(context_id, context_type):
    request_data = request.get_json()
    if not request_data or 'context' not in request_data:
        return jsonify({'error': 'Context data is missing'}), 400
...
@app.route('/save/office/<office_id>', methods=['POST'])
def save_office(office_id):
    return save_generic_context(office_id, "office")

@app.route('/query/office/<office_id>', methods=['POST'])
def query_office(office_id):
    return query_generic_context(office_id, "office")

@app.route('/save/task/<office_id>', methods=['POST'])
def save_task(task_id):
    return save_generic_context(task_id, "task")

@app.route('/query/task/<office_id>', methods=['POST'])
def query_task(task_id):
    return query_generic_context(task_id, "task")

@app.route('/save/faq/<office_id>', methods=['POST'])
def save_faq(faq_id):
    return save_generic_context(faq_id, "faq")
@app.route('/query/faq/<faq_id>', methods=['POST'])
def query_faq(faq_id):
    return query_generic_context(faq_id, "faq")
# Query team context
@app.route('/query/team/<context_id>', methods=['POST'])
def query_team(context_id):
    return query_generic_context(context_id, "team")

# Query office context
@app.route('/query/office/<context_id>', methods=['POST'])
def query_office(context_id):
    return query_generic_context(context_id, "office")

# Query task context
@app.route('/query/task/<context_id>', methods=['POST'])
def query_task(context_id):
    return query_generic_context(context_id, "task")

# Query document context
@app.route('/query/doc/<context_id>', methods=['POST'])
def query_doc(context_id):
    return query_generic_context(context_id, "doc")


# Generic function to query context
def query_generic_context(context_id, context_type):
    request_data = request.get_json()
    if not request_data or 'query' not in request_data:
        return jsonify({'error': 'Query parameter is missing'}), 400
...
def handle_generic_context(context_id, context_type):
    request_data = request.get_json()
    if not request_data or 'query' not in request_data:
        return jsonify({'error': 'Query parameter is missing'}), 400
